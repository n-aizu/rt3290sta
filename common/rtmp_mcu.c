...
#ifdef RT3290
	src = RTMP_3290_FIRMWARE_FILE_NAME;
#else
...
#ifdef RT3290
	if (FIRMWAREIMAGE_LENGTH == FIRMWAREIMAGE_MAX_LENGTH
		||( FIRMWAREIMAGE_LENGTH == FIRMWAREIMAGE_3290_LENGTH && IS_RT3290(pAd)))
#else
	if (FIRMWAREIMAGE_LENGTH == FIRMWAREIMAGE_MAX_LENGTH)
#endif /*RT3290*/
	{
	...
	IN BOOLEAN			FlgIsNeedLocked)
{
	...
	//printk("RtmpAsicSendCommandToMcu Command=0x%x\n",Command);
	if (((IS_RT3090(pAd) || IS_RT3572(pAd) || IS_RT3390(pAd) 
		|| IS_RT3593(pAd) || IS_RT5390(pAd) || IS_RT5392(pAd)
		|| IS_RT5592(pAd)) 
...
#ifdef RT3290
		|| (IS_RT3290(pAd) && (Command == WAKE_MCU_CMD) && (pAd->brt30xxBanMcuCmd == FALSE)
		)
#endif /* RT3290 */
	)
	{
		...
		if ((Command != 0x84) && (Command != WAKE_MCU_CMD))
		{
			do
			{
				...
				RTMPusecDelay(2);
				DBGPRINT(RT_DEBUG_TRACE, ("AsicSendCommanToMcu(1)::Mail box is busy i=%d\n",i));
			} while(i++ < 100);

			if (i >= 100)
			{
				DBGPRINT_ERR(("H2M_MAILBOX still hold by MCU. command fail(1)\n"));
#ifdef RTMP_MAC_PCI
#endif /* RTMP_MAC_PCI */
					return FALSE;
			}
...
#endif /* CONFIG_STA_SUPPORT */
	{
		...
		do
		{
			...
			RTMPusecDelay(2);
			//DBGPRINT(RT_DEBUG_TRACE, ("AsicSendCommanToMcu(2)::Mail box is busy i=%d\n",i));
		} while(i++ < 100);
...
#endif /* RTMP_MAC_PCI */
			{
				DBGPRINT_ERR(("H2M_MAILBOX still hold by MCU. command fail(2)\n"));
			}
#ifdef RTMP_MAC_PCI
#endif /* RTMP_MAC_PCI */
			goto done;
